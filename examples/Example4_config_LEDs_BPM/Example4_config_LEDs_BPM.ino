...
void setup(){
  ...
  int error = max30102.configSensorBpm(MODE_ONE); // Configure Sensor and BPM mode , MODE_TWO also available
  ...
  // Set pulse width.
  error = max30102.setPulseWidth(width);
  if (error == 0){// Zero errors.
  ...
  pulseWidthVal = max30102.readPulseWidth();
  ...
  error = max30102.setSampleRate(samples);
  if (error == 0){// Zero errors.
  ...
  // Check sample rate.
  sampleVal = max30102.readSampleRate();
